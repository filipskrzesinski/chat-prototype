...
function logIn(identity, displayName) {
  fingerprint.get(function(endpointId) {
    request('/getToken?identity=' + identity + '&endpointId=' + endpointId, function(err, res) {
      ...
      $('#profile span').text(client.user.friendlyName || client.user.identity);
...
function createMessage(message, $el) {
  ...
  var $edit = $('<button class="btn btn-primary"/>')
    .text('Make Change')
    .on('click', function(e) {
...
function updateTypingIndicator() {
  ...
  if (typingMembers.size) {
    message = names.join(', ') + ' is typing';
  }

  if (typingMembers.size > 3) {
    message = names.join(', ') + ', and ' + (typingMembers.size-3) + ' more are typing';
  }
